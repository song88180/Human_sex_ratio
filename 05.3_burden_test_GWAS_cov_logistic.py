...
# Load and preprocess phenotype, covariate, and sample data
...
cov_df_raw = pd.read_table('/home/siliang/Public/workspace/4_UKB_merged_files/GWAS/GWAS_adj_01.cov')
...
# Load the variant and sample files for genes
...
# Ensure IID in sam_KM_WES_df and in sam_WES_df have the same order.
...
# Merge phenotype and sample data.
...
# Prepare X (all covariates without genotypes) and Y for logistic regression
...
out_df.to_csv(f'./Gene_OSR_cov_logistic/Gene_OSR_cov_logistic_chr{Chr_idx}.tsv',sep='\t',index=False)
